...
internal/
  config/config.go             # 설정 파싱
  proxy/server.go              # TCP 리스너
  pool/pool.go, health.go      # 커넥션 풀링
  router/router.go, parser.go, balancer.go  # 쿼리 라우팅
  cache/cache.go, invalidator.go            # 쿼리 캐싱
```
...
- 시리즈 포스트: `content/posts/YYYY-MM-DD-db-proxy-N-제목.md`
