...
- **트랜잭션 레벨 커넥션 풀링** — Writer/Reader 모두 커넥션 풀에서 관리합니다. 트랜잭션 시작 시 커넥션을 획득하고 종료 시 반환하여, 수천 클라이언트가 소수의 백엔드 커넥션을 공유합니다. 커넥션 반환 시 `DISCARD ALL`로 세션 상태를 초기화합니다. **Connection Warming** — 프록시 시작 및 config reload 시 `min_connections`까지 백그라운드로 사전 연결하여 cold start tail latency를 완화합니다.
...
- **Query Rewriting Rules** — AST 기반 쿼리 변환 엔진입니다. YAML 설정으로 선언적 규칙을 정의하면 쿼리가 백엔드로 전달되기 전에 자동 변환됩니다. deprecated 테이블명 치환(`table_rename`), 컬럼명 변경(`column_rename`), 조건 자동 추가(`add_where` — 멀티테넌시 격리), 스키마 자동 지정(`schema_qualify`) 등을 지원합니다. `pg_query` AST 파싱 → 노드 수정 → Deparse로 정확한 SQL 재생성을 수행하며, Firewall 검사보다 먼저 적용되어 변환된 쿼리에 방화벽 규칙이 적용됩니다. Simple Query와 Extended Query 프로토콜 모두 지원합니다.
...
- **Per-User / Per-IP Rate Limiting** — 전역 Rate Limit 외에 사용자별·클라이언트 IP별 독립 Token Bucket을 유지합니다. `rate_limit.per_user`, `rate_limit.per_ip`로 활성화하며, `overrides`로 특정 사용자/IP에 차등 제한을 설정할 수 있습니다. 미사용 엔트리는 TTL 기반으로 자동 정리되며, 설정 변경 시 hot-reload로 즉시 반영됩니다.
...
```mermaid
graph LR
    App["App<br/>(pgx, JDBC, ...)"] -- "PG Wire Protocol" --> RL
    HTTP["HTTP Client"] -- "HTTP" --> DAPI

    subgraph pgmux
        DAPI["Data API :8080"] --> RL
        RL["Rate Limit<br/>(Global / User / IP)"] --> AUTH["Auth &<br/>Session Guard"]
        AUTH --> FW["Firewall &<br/>Query Rewrite"]
        FW --> RTR["R/W Router"]
        RTR --> CACHE["Cache (LRU)"]
        RTR --> POOL["Connection Pool<br/>+ Circuit Breaker"]
        CACHE -. miss .-> POOL
        ADM["Admin :9091"]
        MET["Metrics :9090"]
        AUDIT["Audit & Digest"]
...
internal/
  ...
  router/rewriter.go              # Query Rewriting Rules (AST 기반 쿼리 변환 엔진)
  ...
  resilience/ratelimit_registry.go # Per-User / Per-IP Rate Limiter Registry
...
| `/admin/rewrite/rules` | GET | viewer | 활성 Query Rewriting Rules 목록 조회 |
...
- `pgmux_query_rewritten_total` — Query Rewriting Rules에 의해 변환된 쿼리 수 (rule별)
...
- `pgmux_pool_warm_connections_total` — Connection Warming으로 사전 생성된 커넥션 수 (role/addr별)
- `pgmux_pool_warm_errors_total` — Connection Warming 실패 횟수 (role/addr별)
