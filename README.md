...
> CPU 바운드(SELECT-only)에서 pgmux는 Direct의 83% 수준. I/O 바운드(TPC-B)에서는 72%. PgBouncer는 C로 작성된 특화 pooler라 raw throughput에서 유리하지만, pgmux는 캐싱, 방화벽, 미러링 등 PgBouncer에 없는 기능을 제공합니다. 캐시 활성화 시 반복 쿼리는 직접 연결보다 빠릅니다.
>
