...
| P43 | #188 후 | "Go로 PostgreSQL 프록시 만들기 (43) - Idle Client Timeout" | SetReadDeadline, 트랜잭션 중 비적용, FATAL 57P01, hot-reload, PgBouncer client_idle_timeout 대응 | 예정 |
| P44 | #190 후 | "Go로 PostgreSQL 프록시 만들기 (44) - Admin API 인증과 RBAC" | Bearer API Key, admin/viewer 역할 분리, IP allowlist, CIDR 파싱, withAuth 미들웨어, X-Forwarded-For, hot-reload, PgBouncer admin 비교 | 예정 |
