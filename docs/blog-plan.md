...
| P43 | #188 후 | "Go로 PostgreSQL 프록시 만들기 (43) - Idle Client Timeout" | SetReadDeadline, 트랜잭션 중 비적용, FATAL 57P01, hot-reload, PgBouncer client_idle_timeout 대응 | 완료 |
| P44 | #190 후 | "Go로 PostgreSQL 프록시 만들기 (44) - Admin API 인증과 RBAC" | Bearer API Key, admin/viewer 역할 분리, IP allowlist, CIDR 파싱, withAuth 미들웨어, X-Forwarded-For, hot-reload, PgBouncer admin 비교 | 완료 |
| P45 | #192 후 | "Go로 PostgreSQL 프록시 만들기 (45) - Health Check Endpoint와 LB/K8s Probe" | /healthz liveness, /readyz readiness, 인증 우회, Writer TCP 체크, K8s probe 연동, /admin/health 역할 분리 | 완료 |
| P46 | QA 후 | "Go로 PostgreSQL 프록시 만들기 (46) - QA 소견 6건과 운영 안전성 수정" | Pool.Close race, credential reload, XFF spoofing, reader 격리, extended cache, sample_ratio 설정 | 완료 |
| P47 | QA 2차 후 | "Go로 PostgreSQL 프록시 만들기 (47) - QA 2차: Cross-Pool 오염과 캐시 정확성" | boundWriter cross-pool, extended cache 파라미터 무시, Close outstanding borrow, writer CB reload, watcher startup race | 완료 |
| P48 | QA 3차 후 | "Go로 PostgreSQL 프록시 만들기 (48) - QA 3차: 풀 안전성의 마지막 구멍들" | fallback cross-pool, extended cache binary/partial fetch, 깨진 연결 Release, Acquire close race, boundWriter discard 누락, CB 일관성 | 완료 |
| P49 | #223 후 | "Go로 PostgreSQL 프록시 만들기 (49) - Online Maintenance Mode" | atomic 상태 관리, 연결/쿼리 거부, 트랜잭션 drain, /readyz 503 연동, Admin API, PgBouncer PAUSE 비교 | 완료 |
| P50 | #225 후 | "Go로 PostgreSQL 프록시 만들기 (50) - Read-Only Mode" | atomic read-only 플래그, QueryWrite 선별 거부, Simple/Extended 양쪽 커버, ERROR+ReadyForQuery 연결 유지, PgBouncer 비교 | 완료 |
| P51 | #227 후 | "Go로 PostgreSQL 프록시 만들기 (51) - 설정 이중 구조 청산" | writer/readers+databases 이중 구조 통합, ResolvedDatabases() shim 제거, validate 단일 경로, Admin API 응답 정리 | 완료 |
| P52 | #229 후 | "Go로 PostgreSQL 프록시 만들기 (52) - Session Compatibility Guard" | 세션 의존 기능 감지, block/warn/pin/allow 모드, 하이브리드 감지, advisory lock 구분, 커넥션 수명 관리 | 완료 |
| P53 | QA 4차 후 | "Go로 PostgreSQL 프록시 만들기 (53) - QA 4차: 라우팅 우회와 운영 안전성" | Prepared statement read-only 우회, side-effectful SELECT 라우팅, extended/multiplex timeout, Data API hot-reload, per-DB config validation | 완료 |
| P54 | QA 5차 후 | "Go로 PostgreSQL 프록시 만들기 (54) - QA 5차: 릴리즈 위생과 CI 안정성" | dataapi.New 시그니처 불일치, watcher 테스트 race 플래키, fw.Ready() 동기화 | 완료 |
| P55 | QA 6차 후 | "Go로 PostgreSQL 프록시 만들기 (55) - QA 6차: 파서 우회와 분류 사각지대" | 주석 우회 tx/session bypass, MERGE/COPY/CALL 분류 누락, advisory lock/side-effect false positive | 완료 |
| P56 | QA 7차 후 | "Go로 PostgreSQL 프록시 만들기 (56) - QA 7차: 릴리즈 전 최종 코드 리뷰" | CopyBoth race, rate limiter clock skew, SQL 에러 노출, 캐시 인덱스 파손, synthesizer 메모리, EXPLAIN ANALYZE/ABORT/COPY/SET CONSTRAINTS 파서 일관성 | 완료 |
| P57 | v1.0.0 후 | "Go로 PostgreSQL 프록시 만들기 (57) - v1.0.0 릴리즈 준비" | CHANGELOG.md 작성, README 9곳 동기화, 릴리즈 체크리스트, 시리즈 마무리 | 완료 |
| P58 | #260 후 | "Go로 PostgreSQL 프록시 만들기 (58) - SQL Redaction과 Safe Observability" | pg_query.Normalize 기반 리터럴 마스킹, 세 가지 정책, 15개 노출 지점 일괄 적용 | 완료 |
| P59 | QA 8·9차 후 | "Go로 PostgreSQL 프록시 만들기 (59) - QA 8·9차: COPY 방화벽 우회부터 타이밍 공격까지" | COPY firewall bypass, SET TRANSACTION false positive, CALL 캐시, pool mutex 블로킹, auth timing attack, extIsWrite 미초기화, lint 정리 | 완료 |
| P60 | #268,#270 후 | "Go로 PostgreSQL 프록시 만들기 (60) - Extended Query 커넥션 바인딩과 E2E 전수 검증" | lib/pq 2-round 프로토콜 바인딩 버그, extHasExecute, boundReader/Writer, 22개 E2E 테스트, 8개 신규 기능 검증 | 완료 |
| P61 | #274 후 | "Go로 PostgreSQL 프록시 만들기 (61) - E2E 커버리지 갭 분석과 미커버 기능 테스트" | 전체 기능 대비 커버리지 갭 분석, Route Hint·Timeout Hint·Digest Reset·Semantic Cache·Slow Query·Config Reload·CancelRequest·Mirror Stats 8개 테스트 추가 (22→30개) | 완료 |
| P62 | #276 후 | "Go로 PostgreSQL 프록시 만들기 (62) - 릴리스 전 최종 점검: 5개 버그와 품질 개선" | Writer pool nil deref, Synthesizer LRU, Redis Ping timeout, Extended Query DoS, TLS fail-fast, CopyBoth drain, 패키지 doc comment | 완료 |
